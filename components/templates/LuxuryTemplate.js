    ...
    <main dir="rtl" className="min-h-screen bg-[#080604] text-[#f5ead8]">
      <section className="px-5 py-10">
        <div className="mx-auto max-w-5xl">
          {menu.logo_url && (
            <img src={menu.logo_url} alt={menu.name} className="mx-auto h-28 w-28 rounded-full object-cover" />
          )}
              ...
              <section key={section.id}>
                ...
                <div className="mt-6 grid gap-5 md:grid-cols-2">
                  {(section.items || []).map((item) => (
                    <article key={item.id} className="rounded-[2rem] border border-[#f5ead8]/15 p-5">
                      {item.image_url && (
                        <img src={item.image_url} alt={item.name_ar} className="mb-4 h-52 w-full rounded-[1.5rem] object-cover" />
                      )}
                      ...
                      {item.description_ar && (
                        <p className="mt-3 text-sm text-[#f5ead8]/55">{item.description_ar}</p>
                      )}
                    </article>
                  ))}
                </div>
